pipeline {
  environment {
    registry = "romancin/traefik-custom-error-pages"
    repository = "traefik-custom-error-pages"
  ...
  stages {
    stage('Cloning Git Repository') {
      steps {
        git url: 'https://github.com/romancin/traefik-custom-error-pages.git',
            branch: '$BRANCH_NAME'
      }
    }
      ...
      steps {
        script {
          ...
          docker.withRegistry('', registryCredential) {
            def image = docker.build("$registry:$gitbranch",  "-f .cloud/docker/Dockerfile .")
            image.push()
            image.push(major)
            image.push(minor)
            image.push(patch)
    ...
    stage('Building image and pushing it to the registry (master)') {
      ...
      steps {
        script {
          ...
          docker.withRegistry('', registryCredential) {
            def image = docker.build("$registry:latest", "-f .cloud/docker/Dockerfile .")
            image.push()
            image.push(major)
            image.push(minor)
            image.push(patch)
  ...
  post {
        success {
            telegramSend(message: '[Jenkins] - Pipeline CI-traefik-custom-error-pages $BUILD_URL finalizado con estado :: $BUILD_STATUS', chatId: -395961814)
        }
  }
 }
